...
function render() {
  ...
  $$('[data-m]').forEach(i => i.onchange = () => { const r = R.find(x=>x.id===+i.dataset.r); ensureOwn(r); let m = C.state.maatregelen.find(x=>x.id===i.dataset.m) || C.state.maatregelen.find(x=>x.regelId===r.id); const k=i.dataset.k; const old=m[k]; m[k] = k==='handeling' ? i.value : C.numNL(i.value); m.bron='mens'; C.audit({regel:r.id, veld:'mjop.'+k, oud:old, nieuw:m[k], bron:'mens'}); C.save(); window.STEMI_UI.renderAll('mjop'); });
