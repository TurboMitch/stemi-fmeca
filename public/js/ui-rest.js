...
function renderOverzicht() {
  ...
  $('#tab-overzicht').innerHTML = `
    <h2>Overzicht</h2><p class="sub">Instellingenprofiel <b>${esc(S.naam)}</b> ${S.wizardVoltooid?'<span class="tag">vastgelegd</span>':'<span class="warn">nog niet vastgelegd via de AM-wizard</span>'} · profiel <b>${esc(S.params.profiel)}</b> · start ${S.params.startjaar} · horizon ${S.params.horizon} jaar · O-referentie ${S.params.oRef} jaar</p>
...
function exportXlsx() {
  ...
  const s06 = [['Beslisregels (JSON)'],[JSON.stringify(S.rules)],[],['O-score','Kans','Betekenis'],...S.scorekaarten.O.map(o=>[o.score,o.kans,o.betekenis]),[],['D-score','Detecteerbaarheid','Criterium'],...S.scorekaarten.D.map(d=>[d.score,d.detect,d.criterium]),[],['S-score','Generiek',...ASP],...S.scorekaarten.S.map(s=>[s.score,s.generiek,...s.aspecten])];
