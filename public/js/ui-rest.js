...
function renderSysteem() {
  ...
  $('#tab-systeem').innerHTML = `
    ...
    <div class="tablewrap"><table><thead><tr><th>ID</th><th>Element</th><th>O</th><th>D</th><th>T</th>${ASP_SHORT.map(a=>`<th>E ${esc(a)}</th>`).join('')}${ASP_SHORT.map(a=>`<th>W ${esc(a)}</th>`).join('')}<th>S tech</th><th>RPN tech</th><th>S waarde</th><th>RPN waarde</th><th>Basis</th><th>Safety</th><th>Compl.</th><th>T-prio</th><th>NEN signaal</th><th>Definitief</th><th>Uiterlijk</th><th>Deadline</th><th>Dominant tech</th><th>Dominant waarde</th><th>Kosten</th><th>Bron</th><th>Status</th></tr></thead><tbody>
...
function renderInstellingen() {
  ...
  $('#tab-instellingen').innerHTML = `
    ...
    <div class="tablewrap"><table><thead><tr><th>Code</th><th>Bouwdeel</th><th>Klasse</th><th class="wrap">Omschrijving</th><th class="wrap">Faalwijze (voorstel)</th>${ASP_SHORT.map(a=>`<th>${esc(a)}</th>`).join('')}<th>Vertr.</th><th></th></tr></thead><tbody>
  ...
  $('#upJson').onchange = e => { const f=e.target.files[0]; if(!f) return; f.text().then(t=>{ const st = JSON.parse(t); if(!st.inspectie) throw new Error('geen dataset'); if (!confirm('De huidige projectdata wordt vervangen door dit bestand. Doorgaan?')) return; C.setState(st); C.save(); window.STEMI_UI.renderAll(); C.toast('Dataset geladen'); }).catch(err=>alert('Ongeldig bestand: '+err.message)); };
