...
function defaultRules() {
  return {
    ...
    oVoorstel: { basis:3, intensiteit:{'Eindstadium':3,'Gevorderd':2,'Duidelijk waarneembaar':1,'Beginstadium':0}, omvang:[{min:0.5,add:2},{min:0.2,add:1}], conditieVanaf:5, conditieAdd:1 },
    dVoorstel: {'Volledig':2,'Deels':7,'Niet':8},
...
function systeemvoorstelO(insp, omvang) {
  const R = state.settings.rules.oVoorstel; let o = R.basis + (R.intensiteit[insp.intensiteit] ?? 0);
  const omv = num(omvang) ?? 0; const b = R.omvang.find(x => omv >= x.min); if (b) o += b.add;
  if ((num(insp.conditie) ?? 0) >= R.conditieVanaf) o += R.conditieAdd;
...
function voorstelDtekst(insp) { return insp==='Volledig'?'Goed zichtbaar bij reguliere inspectie.' : insp==='Deels'?'Kritische delen deels verborgen; specialistische beoordeling kan nodig zijn.' : 'Niet regulier zichtbaar; specialistisch onderzoek/monitoring nodig.'; }
...
const OKANS = () => state.settings.scorekaarten.O.map(o=>o.kans);
...
function recompute() {
  ...
  for (const insp of state.inspectie) {
    ...
    const oSys = systeemvoorstelO(insp, omvang), dSys = voorstelD(insp.inspecteerbaarheid);
    ...
    const safety = prioFromThresholds(R.safety, effect[0], '') || '';
    const compliance = prioFromThresholds(R.compliance, effect[1], '') || '';
    ...
    const r = { id: insp.id, insp, sp, bes, libE, omvang, kostenElement, kostenLokaal, omslagEff, begrotingswijze, begrHoev, eersteVoorstel,
      oSys, dSys, tKlSys, tJaarSys, tTxtSys, O, D, Tklasse, Tjaar, effect, faalwijze, kostenSpec, definitieveKosten,
...
return { ASP, ASP_SHORT, INTENSITEIT, ERNST, INSPECTEERBAAR, PRIOS, MODELS, SP_NUM, SP_FIELDS,
